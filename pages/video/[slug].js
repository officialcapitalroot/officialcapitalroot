...
export default function VideoPage({ 
  ...
  // FIXED: Proper canonical URL handling
  const getCanonicalUrl = () => {
    return `https://capitalroot.vercel.app/video/${slug}`;
  };
  ...
  // FIXED: Better thumbnail handling with absolute URLs
  const getThumbnailUrl = () => {
    if (isTMDB) {
      if (media.poster_path || media.backdrop_path) {
        return getImageUrl(media.poster_path || media.backdrop_path, "w500");
      ...
      if (video.thumbnail.startsWith('http')) {
        ...
        return `https://capitalroot.vercel.app${video.thumbnail.startsWith('/') ? video.thumbnail : '/' + video.thumbnail}`;
      }
    }
  };
  ...
  // Get video player for JSON videos
  const getJSONVideoPlayer = () => {
    if (video.videoSource === "dailymotion") {
      return (
        <DailymotionPlayer 
  ...
  // FIXED: Improved structured data
  const getStructuredData = () => {
    const baseSchema = {
      "@context": "https://schema.org",
      "name": currentMedia.title || currentMedia.name,
  ...
  // FIXED: Improved meta description
  const getMetaDescription = () => {
    const baseDescription = `Watch ${currentMedia.title || currentMedia.name} online in HD quality for free. `;
    const overview = isTMDB ? media.overview : video.description;
    
    if (overview && overview.length > 120) {
      return baseDescription + overview.substring(0, 120) + '...';
    }
    return baseDescription + (overview || `Stream ${currentMedia.title || currentMedia.name} now.`);
  };
  ...
  return (
    <>
      <Head>
        {/* FIXED: Improved title and meta tags */}
        <title>{currentMedia.title || currentMedia.name} - Watch {mediaType === 'tv' ? 'TV Show' : 'Movie'} Online | Capital Root Movies</title>
        <meta name="description" content={metaDescription} />
        
        <meta name="keywords" content={`${currentMedia.title || currentMedia.name}, watch online, free streaming, ${isTMDB ? media.genres?.map(g => g.name).join(', ') : video.tags?.join(", ") || ""}, ${mediaType === 'tv' ? 'tv series' : 'movies'}, HD quality`} />
        
        <link rel="canonical" href={canonicalUrl} />

        {/* Open Graph */}
        <meta property="og:title" content={`${currentMedia.title || currentMedia.name} - Watch Online`} />
        ...
        {/* Twitter Card */}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={`${currentMedia.title || currentMedia.name} - Watch Online`} />
      ...
      <main className="video-page-main">
        <div className="container">
          <nav aria-label="Breadcrumb" className="page-navigation">
            <button
              onClick={() => router.push(mediaType === 'tv' ? "/tv" : "/videos")}
              className="back-button"
              aria-label={`Go back to all ${mediaType === 'tv' ? 'TV shows' : 'movies'}`}
            >
...
export async function getStaticPaths() {
  ...
  try {
    ...
    const moviePaths = trendingMovies.results.slice(0, 50).map((movie) => ({
      params: { slug: `movie-${movie.id}` },
    }));

    const tvPaths = trendingTV.results.slice(0, 50).map((tv) => ({
      params: { slug: `tv-${tv.id}` },
    }));
