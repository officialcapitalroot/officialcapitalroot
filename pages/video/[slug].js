...
//   // FIXED: Truncate description for Twitter (max 200 chars)
//   const truncateDescription = (desc) => {
//     if (!desc) return '';
//     return desc.length > 200 ? desc.substring(0, 200) + '...' : desc;
//   };

//   const pageTitle = `${currentMedia.title || currentMedia.name} - ${mediaType === 'tv' ? 'Watch TV Show' : 'Watch Online'} | Capital Root Movies`;
//   const pageDescription = truncateDescription(isTMDB ? media.overview : video.description);
...
//         {/* FIXED: Twitter Card Meta Tags - COMPLETELY REWRITTEN */}
//         <meta name="twitter:card" content="summary_large_image" />
//         <meta name="twitter:site" content="@capitalroot" />
...
//         <meta name="twitter:image" content={thumbnailUrl} />
//         <meta name="twitter:image:alt" content={`Poster for ${currentMedia.title || currentMedia.name}`} />
//         <meta name="twitter:domain" content="capitalroot.vercel.app" />

//         {/* Additional Twitter Player Card for videos */}
//         {!isTMDB && video.videoSource === "youtube" && (
//           <>
//             <meta name="twitter:player" content={`https://www.youtube.com/embed/${video.videoId}`} />
//             <meta name="twitter:player:width" content="1280" />
//             <meta name="twitter:player:height" content="720" />
//             <meta name="twitter:player:stream" content={video.videoUrl} />
//           </>
//         )}
...
export default function VideoPage({ 
  ...
  const truncateDescription = (desc) => {
    if (!desc) return 'Watch this video on Capital Root Movies';
    return desc.length > 200 ? desc.substring(0, 200) + '...' : desc;
  };
  ...
  return (
    <>
      <Head>
        ...
        <meta name="keywords" content={`${currentMedia.title || currentMedia.name}, watch online, stream, ${isTMDB ? media.genres?.map(g => g.name).join(', ') : video.tags?.join(", ") || ""}, ${mediaType === 'tv' ? 'tv shows' : 'movies'}`} />
        <link rel="canonical" href={canonicalUrl} />
        ...
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(getStructuredData()) }} key="structured-schema" />
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbSchema) }} key="breadcrumb-schema" />
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(videoObjectSchema) }} key="videoobject-schema" />
      </Head>
